...
if [ "${BUILD_STATUS:-PASS}" != "PASS" ]; then
  ...
  cat >> email_body.html <<EOF
          ...
          <p style="margin: 0 0 16px 0; color: #d4d4d4 !important;">$([ "${BUILD_STATUS}" = "FAIL" ] && echo "A failed build means deployment to Cloudflare Workers will be impossible. Production is completely blocked." || echo "Large bundle sizes slow down user engagement (LCP will drop) resulting in severe SEO penalties.")</p>
          <h3 style="font-size: 12px; margin: 0 0 8px 0; color: #A3A3A3 !important; text-transform: uppercase; letter-spacing: 0.05em;">Mitigation Instructions</h3>
