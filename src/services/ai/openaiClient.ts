...
class OpenAIClient {
    ...
    async generateCompletion(
        ...
        if (!this.client) {
            console.error('OpenAI API Key missing. Strict security mode engaged: Mock responses are disabled.');
            throw new AIServiceError('OpenAI API Key is not configured. AI services are offline.', 'CONFIG_MISSING');
        }
        ...
        try {
            ...
            // Fallback to mock removed for strict security. Throw real errors for 401/403.
            if (error instanceof OpenAI.APIError && (error.status === 401 || error.status === 403)) {
                throw new AIServiceError('Invalid or Expired OpenAI API Key.', 'AUTH_FAILED');
            }
