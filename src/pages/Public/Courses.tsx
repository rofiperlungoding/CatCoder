    ...
    return (
        <div className="pt-32 pb-20 space-y-16 px-6 md:px-12 max-w-7xl mx-auto">
            ...
            <div className="text-center space-y-6 max-w-3xl mx-auto">
                <Badge className="bg-lime-100 text-lime-800 hover:bg-lime-200 border-0">Catalog</Badge>
                <h1 className="text-4xl md:text-5xl font-extrabold text-primary animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150 fill-mode-backwards">
                    Explore our <span className="text-lime-500">Curriculum</span>.
                </h1>
                ...
                <div className="relative max-w-lg mx-auto mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300 fill-mode-backwards">
                    ...
                    <input
                        ...
                        className="w-full pl-12 pr-4 py-4 rounded-full border border-gray-200 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-lime-500/20 focus:border-lime-500 transition-all"
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredCourses.map((course) => (
                    <div key={course.id} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-black/5 transition-all duration-300 hover:-translate-y-1 flex flex-col h-full">
                        <div className="flex justify-between items-start mb-6">
                            <div className={`p-4 rounded-2xl ${course.bg} ${course.color} bg-opacity-50`}>
                                {course.icon}
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-8">
                            {course.tags.map(tag => (
                                <span key={tag} className="px-3 py-1 bg-gray-50 text-gray-600 text-xs font-semibold rounded-full border border-gray-100">
                                    {tag}
                                </span>
                            ))}
                        </div>
                        ...
                        <div className="mt-auto space-y-6">
                            ...
                            <Button className="w-full rounded-full group" onClick={() => navigate('/learn')}>
